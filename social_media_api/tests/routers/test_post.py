...
async def test_create_post_expired_token(
    async_client: AsyncClient, registered_user_fixture: dict, mocker
):
    mocker.patch(
...
async def test_get_all_posts(async_client: AsyncClient, created_post: dict):
    response = await async_client.get("/get-all-posts")

    assert response.status_code == 200

    posts = response.json()

    assert any(post["body"] == created_post["body"] for post in posts)
...
async def test_get_missing_post_with_comments(
    ...
    missing_post_id = created_post["id"] + 1

    response = await async_client.get(f"/post/{missing_post_id}")
